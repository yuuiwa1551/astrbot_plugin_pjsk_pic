...
## 2026-08-31 首轮真实 shadow 结果

- provider：`volcengine-agent-plan/doubao-seed-2.0-lite`。
- 先选取 5 张已有人工审核痕迹的本地图片，只写 `llm_image_review_runs`，没有改变任何 `review_tasks` / `image_tags` 状态。
- 提示词 v1 中，3/5 响应通过严格契约；另 2 张正确表达“没有候选角色”，但自创 no-match flag，被 parser 安全拦截。
- 提示词 v2 完整列出允许的 flags 并明确无匹配返回方式后，同一组 5/5 全部通过结构化契约。
- 两张单角色人工通过图片均选中相同角色并达到当前自动阈值；两张仅人工拒绝候选的图片均返回空角色且没有达到自动通过条件。
- 一张多人图返回画面中的两名实际角色；历史人工结果使用的是 CP tag，不能直接作为角色多标签的错误样本。该图片也因质量 / 置信度阈值未达到自动通过条件。
- v2 首次启动捕获到 provider 管理器尚未加载完成的短暂 `provider not found`，第二次尝试成功；`v0.19.2` 已增加启动延迟和跨周期重试。
- 结论：真实多模态路径、`file:///` 预览、候选 ID、质量分、空角色、多角色和重试历史均已验证，但 5 张样本不足以授权自动通过。live 保持 `shadow`、关闭新图自动排队，继续等待扩大到至少 50 张的校准。

...
- 内置 PJSK 26 位主要角色的日文查询/匹配词，兼容中文图库名；已有手动平台词继续叠加，保留 `user` 质量初筛后缀
...
- 出现暂停时先区分网络、接口鉴权与账号登录；CLI sidecar 扫码后需要重新加载 Cookie，见[运行恢复说明](docs/xiaohongshu-provider.md#cli-sidecar-运行恢复)。
...
- 白名单群图片独立进入收图管线，由独立审核 worker 处理，不依赖主聊天是否触发
- 审核通过的候选在群内 @ 发图人确认，每次询问逐张附上图片、编号和识别标签，确认后以 `approved` 状态写入图库
- 只有发图人本人的回复生效：引用对应确认消息回复「确认 / 全部收」收录本批，「收第N张」只收该张，「不要」拒绝剩余候选；只有一批待确认时也可 @bot 回复
- 支持「第N张是<tag>」只更正并收录该张，其他图片仍需确认；单图可回复「这张是奏和真冬，收了」；部分确认后仍沿用原消息的编号，多批待确认时必须引用对应消息，不自动选择最新一批
- 成功回执逐张提供真实图库图片 ID 和标签；重复图给出已有 ID，失败项保留原编号。回执不再重复发送图片
- 确认预览文件缺失时转入 WebUI 待审，不发送没有对应图片的收录询问；旧/过期引用不会误确认其他批次
- 独立识图保存每个角色的置信度与简短可见依据，以及质量结果、调用次数、耗时和错误分类；人工更正不覆盖原始预测
- 群聊置信度策略默认 `shadow`：只记录建议确认/待审，不改变模型当前判定与带图人工确认流程；历史无记录的置信度保持未知
- 完整 JSON 代码块可本地解析；非法分数、未知角色和残缺结果不会静默通过。重试使用持久预算与退避，重启不重置调用次数
- 同 tag 集合的候选合并成一条消息展示；超时未回复的候选转入 WebUI 待处理
- 可选自动批准开关：审核通过后跳过群内确认直接入库（默认关闭）
- 主聊天只保留图片问答注入，不再注册收图工具或发送收图回执
...
数据库图片 ID 可使用 `.pp 看图 <image_id>`，也可自然语言发送 `看图<image_id>` 或 `看看id<image_id>` 查看（沿用 ID 查看权限）。纯数字写法如 `看看1` 不再代表最近展示列表序号。
...
- `chat_image_collection_enabled`
  - 是否启用群聊收图独立管线，默认关闭
- `chat_image_collection_groups`
  - 允许收图的群号，多个用英文逗号分隔；留空表示所有群，私聊始终不启用
- `chat_image_collection_attach_originals`
  - 上下文只有图片描述但能定位到原图时，是否把原图补入当前多模态请求
- `chat_image_audit_interval_seconds` / `chat_image_audit_max_per_cycle`
  - 审核 worker 的轮询间隔（默认 `60` 秒）与每轮处理上限（默认 `3`）
- `chat_image_audit_prompt_version`
  - 审核提示词版本标记，用于区分不同审核口径的记录
- `chat_image_confirm_interval_seconds` / `chat_image_confirm_max_per_cycle`
  - 群内确认 worker 的轮询间隔（默认 `60` 秒）与每轮发送批次数（默认 `3`）
- `chat_image_confirm_timeout_hours`
  - 等待群内确认的超时时间（小时），超时后候选转入 WebUI 待处理，默认 `24`
- `chat_image_confirm_batch_size`
  - 单条确认消息最多展示的候选图片数量，默认 `10`
- `chat_image_confirm_receipt_enabled`
  - 写库成功后是否发送包含图片 ID、标签及重复/失败结果的群内回执，默认开启
- `chat_image_audit_identity_mode` / `chat_image_audit_identity_threshold`
  - 默认 `shadow` / `0.93`，仅用于记录群聊置信度分流建议，不是校准后的正确率，也不等于自动收录门槛；`off` 停止生成建议，仍保存识别记录
  - 与图库审核的身份阈值独立，不提供未经评估的自动执行分流模式
  - `/api/chat-candidates?status=audit_error` 可查看错误候选；也支持 `pending_webui`（默认）、`audited`、`asked`、`approved_written`、`rejected`、`captured`、`auditing`
  - 接口的 `audit_identity` 包含角色、依据、Provider、提示词版本、`call_count`、调用耗时、错误类别与 shadow 建议；空历史记录显示 `identity_state=unknown`
  - `call_count` 只统计新增的插件层请求，不冒充 Provider/SDK 的实际 HTTP 重试次数；升级前失败次数单独计入 `legacy_attempt_count` 以保守占用预算
- `chat_image_audit_provider_id`
  - 群聊收图的独立视觉模型 ID；留空沿用图库审核模型，设置后仅影响群聊识图
- `chat_image_confirm_accept_words` / `chat_image_confirm_reject_words`
  - 自定义确认词 / 拒绝词，多个用逗号分隔；留空使用内置默认词
- `chat_image_auto_approve_enabled`
  - 自动批准开关：审核通过后跳过群内确认直接入库，默认关闭
...
- QQ 引用确认支持「收1、3」「跳过2」「收全部」，越界不部分执行，重复编号只处理一次，原图片序号保持不变。
- 「第2张是白石杏和小豆泽心羽，收了」只更正并收录第2张；不再连带确认其他图片。单图支持「这张是…，收了」，多图须写编号。
- 回执显示实际图库 ID、剩余待确认数及「看图1234」用法；新增简写复用原来的入口开关和管理员权限，不开放管理信息。
- 每次确认仍附全部图片；引用必须来自原批次、原群和原发图人。合并转发模式、失败重试和撤销投稿留待后续。

### v0.27.0

- 29 期 B 阶段准备：增加 26 人可溯源文字特征档案及 `chat_image_audit_profile_mode=off/text`，默认关闭，关闭时保持原提示词。
- 新增只读样本导出、离线人工标注页及评估工具；预填建议不会自动变成标准答案，未人工核实或分组存在泄漏时拒绝评分。
- 标注页支持草稿、分批导入导出、目标/非目标与待定状态；样本和私人图片不包含在源码发布中。
- 本阶段尚无人工核实的对照结论，不宣称准确率提升，不启用第二模型或裁切复核。用法见 [离线核实与评估](tools/IDENTITY_REVIEW.md)。

### v0.26.0

- 29 期 A 阶段：保存群聊角色置信度、可见依据、模型与质量判定、调用耗时和错误类别，新增兼容 JSON 字段，保留原始预测与人工更正。
- 只接受有限、合法范围内的评分；本地去除完整 JSON 代码块，不截取任意正文中的局部结果。
- 发请求前持久记录调用预算，重启/取消后不额外获得额度；限流、超时、契约错误退避重试，缺图、鉴权、套餐失效及无效请求不反复调用。
- 新置信度策略只做 shadow，保留原有质量筛选、收图确认、图片 ID 回执及 Pixiv `user` 后缀。
- 支持群聊独立 Provider，替换群聊识图模型时不连带修改图库审核模型。
- 当前 Provider 路径不透传 `response_format`，因此使用提示词 JSON 契约，未宣称启用 JSON Schema；参考图、角色档案和第二模型尚未上线。

### v0.25.2

- 群聊收图确认每次逐张附图，编号、识别标签与本地候选一致；OneBot 与框架消息发送均保留图片段。
- 收录成功回复实际图库 ID；重复图返回已有 ID，部分失败不影响其他图片的编号。
- 支持引用「收第N张 / 全部收」，多批待确认不再默认选最新一批；过期引用、跨群和非发图人回复不能确认新候选。
- 保留自动批准、WebUI 待审、原有识别阈值和 Pixiv 质量初筛策略。

### v0.25.1

- 补齐已有 PJSK 26 位主要角色的日文 Pixiv 查询/匹配映射，覆盖繁简体图库命名差异。
- 修正宵崎奏、朝比奈まふゆ、桐谷遥的首选搜索词；已有自定义平台词继续有效。
- 保留 `user` 搜索后缀作为质量初筛，频率、任务预算、过滤与审核策略保持不变。
- 既有部署中的 `Yoisaki Kanade`、`朝比奈真冬`、`桐谷遙` 如仅需识别旧标签，可改为 `match` 类型；词级历史水位无需删除。

### v0.25.0

- 群聊收图改为独立管线：白名单群收到图片即捕获、预下载并进入独立 LLM 审核，不再依赖主聊天轮次
- 审核通过后群内 @ 发图人确认，仅发图人本人的引用 / @bot 回复生效；支持整批确认、整批拒绝与「第N张是<tag>」更正
- 超时未确认或自动驳回的候选转入 WebUI「群聊候选」页兜底，可手动入库或忽略
- 支持将历史已入库的群聊自动收图回填到 WebUI「群聊候选」页复核：通过不重复入库，忽略则对应关联标记为人工拒绝
- 提供自动批准开关（默认关闭）：审核通过后跳过群内确认直接入库
- 主聊天退出收图：移除 `save_chat_image` 工具与收图回执，仅保留图片问答原图注入
- 新增审核 / 确认 worker 的轮询间隔、每轮上限、确认超时、批次大小、回执开关与自定义确认词等配置项

### v0.24.1

- 修复预下载任务对象混入图片元数据导致收图写库到 `sources` 表序列化失败（`保存失败：TypeError`）的问题
- 预下载任务改为按图片引用单独记账，元数据保持可 JSON 序列化；补回归断言

### v0.24.0

- 白名单群收到图片后尽早预下载到插件临时缓存（内容 SHA 命名），收图不再依赖会过期的 QQ URL
- 当前图、引用图与历史补图统一使用同一份已解析图片数据；历史图片注释在请求副本里归一化，不再逐轮累加
- 只处理当前消息与本轮新增的上下文图片，历史图片正常显示但不默认进入收图候选，已有 ref 不再重新提名
- 永久失败的来源记录一次原因后退出本轮与后续旧候选，新消息带新下载凭据时可重新解析
- 回复引用图保留被引用消息的来源与发送者，不冒充当前说话人
- 成功率无关的失败不丢弃普通聊天内容；插件启动时清理超过保留期的临时缓存
- 保留生命周期与收图专项回归，新增缓存去重、坏链接、引用来源与重复注释回归

### v0.23.4

- 历史补图优先使用已解析本地原图，以图片数据发送给模型，不直接追加 QQ 下载链接
- 不可用历史图片从本次请求跳过，不影响当前消息原有的图片处理流程
- 收图入库复用已解析的本地文件，避免对同一张图重新请求 QQ URL
